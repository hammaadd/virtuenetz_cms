...
if(options.evalBeforeUpdate&&$.proxy(new Function(options.evalBeforeUpdate),$el)()===false){return;}
...
var updatePromise=requestOptions.handleUpdateResponse(data,textStatus,jqXHR);updatePromise.done(function(){$triggerEl.trigger('ajaxSuccess',[context,data,textStatus,jqXHR]);options.evalSuccess&&$.proxy(new Function(options.evalSuccess),$el)();})
...
updatePromise.done(function(){$el.data('error-message',errorMsg);var _event=jQuery.Event('ajaxError');$triggerEl.trigger(_event,[context,errorMsg,textStatus,jqXHR]);if(_event.isDefaultPrevented())return;if(options.evalError&&$.proxy(new Function(options.evalError),$el)()===false){return;}
requestOptions.handleErrorMessage(errorMsg);});return updatePromise;},complete:function(data,textStatus,jqXHR){$triggerEl.trigger('ajaxComplete',[context,data,textStatus,jqXHR]);options.evalComplete&&$.proxy(new Function(options.evalComplete),$el)();},handleConfirmMessage:function(message){var _event=jQuery.Event('ajaxConfirmMessage');_event.promise=$.Deferred();if($(window).triggerHandler(_event,[message])!==undefined){_event.promise.done(function(){options.confirm=null;new Request(element,handler,options);})
return false;}
