...
$.oc={}
$.oc.alert=function alert(message,title){var messageTitle=typeof title!=='string'?$.oc.lang.get('alert.error'):title
$.oc.vueComponentHelpers.modalUtils.showAlert(messageTitle,message,{buttonText:$.oc.lang.get('alert.dismiss')})}
$.oc.confirm=function confirm(message,callback,title){var messageTitle=typeof title!=='string'?$.oc.lang.get('alert.confirm'):title
$.oc.vueComponentHelpers.modalUtils.showConfirm(messageTitle,message,{}).then(function(){callback(true)},function(){})}})(jQuery);$(window).on('ajaxErrorMessage',function(event,message){if(!message)return
$.oc.alert(message)
...
+(function($){'use strict';if($.oc===undefined)$.oc={};var Snackbar=function(){var queue=[],lastUniqueId=0,displayedMessage=null;function validateOptions(options){if(options===undefined){return;}
if(typeof options!=='object'){throw new Error('Snackbar options must be an object');}
if(options.replace!==undefined){if(typeof options.replace!=='number'){throw new Error('Snackbar options.replace must be a number');}}
if(options.timeout!==undefined){if(typeof options.timeout!=='number'){throw new Error('Snackbar options.timeout must be a number');}
if(options.timeout<2000){throw new Error('Snackbar options.timeout cannot be lower than 2000');}
if(options.timeout>8000){throw new Error('Snackbar options.timeout cannot be higher than 8000');}}
if(options.action!==undefined){if(typeof options.action!=='object'){throw new Error('Snackbar options.action must be an object');}
if(typeof options.action.label!=='string'){throw new Error('Snackbar options.action.label must be a string');}
if(typeof options.action.callback!=='function'){throw new Error('Snackbar options.action.callback must be a function');}}}
function runQueue(){if(displayedMessage!==null){return;}
var parameters=queue.shift();if(parameters===undefined){return;}
buildSnackbar(parameters);}
function makeUniqueId(){return++lastUniqueId;}
function buildSnackbar(parameters){displayedMessage={uniqueId:parameters.uniqueId};var $element=$('\
                    <div class="october-snackbar"> \
                        <div class="snackbar-label"></div> \
                        <button class="snackbar-dismiss" aria-title="Dismiss">X</button> \
                    </div>'),$label=$element.find('.snackbar-label');$label.text(parameters.message);$element.find('.snackbar-dismiss').one('click.snackbar',function(){hideSnackbar($element,parameters.options);});if(parameters.options&&parameters.options.action){var $actionButton=$('<button class="snackbar-action"></button>');$actionButton.text(parameters.options.action.label);$actionButton.one('click.snackbar',function(){parameters.options.action.callback();hideSnackbar($element,parameters.options);});$actionButton.insertAfter($label);}
$(document.body).append($element);displayedMessage.element=$element;displayedMessage.uniqueId=parameters.uniqueId;displayedMessage.options=parameters.options;setTimeout(function(){$element.addClass('enter');setTimeout(function(){$element.addClass('show-snackbar');startHideTimeout($element,parameters.options);},20);},1);}
function startHideTimeout($element,options){var timeout=2000;if(options&&options.timeout){timeout=options.timeout;}
options.timeoutId=setTimeout(function(){hideSnackbar($element,options);},timeout);}
function hideSnackbar($element,options){$element.find('button').off('.snackbar');$element.removeClass('show-snackbar');clearTimeout(options.timeoutId);setTimeout(function(){displayedMessage=null;$element.remove();runQueue();},210);}
function forceHideSnackbar($element,options){clearTimeout(options.timeoutId);options.timeoutId=setTimeout(function(){hideSnackbar($element,options);},200);}
function removeFromQueue(uniqueId){for(var index=0;index<queue.length;index++){if(queue[index].uniqueId==uniqueId){queue.splice(index,1);return;}}}
this.show=function(message,options){validateOptions(options);if(options&&options.replace){if(displayedMessage&&options.replace===displayedMessage.uniqueId){forceHideSnackbar(displayedMessage.element,displayedMessage.options);}
else{removeFromQueue(options.replace);}}
var uniqueId=makeUniqueId();queue.push({message:message,options:options||{},uniqueId:uniqueId});runQueue();return uniqueId;};this.hide=function(uniqueId){if(displayedMessage&&uniqueId===displayedMessage.uniqueId){forceHideSnackbar(displayedMessage.element,displayedMessage.options);}};};$.oc.snackbar=new Snackbar();})(window.jQuery);+function($){"use strict";var Base=$.oc.foundation.base,BaseProto=Base.prototype
...
return this}
$(document).on('render',function(){$('div[data-control=scrollpad]').scrollpad()})}(window.jQuery);+function($){"use strict";if($.oc===undefined)
$.oc={}
...
this.$list.dragScroll({vertical:true,useNative:false,useDrag:true,start:function(){self.$list.addClass('drag')},stop:function(){self.$list.removeClass('drag')},scrollClassContainer:self.$el,scrollMarkerContainer:self.$el})
...
function setCounterValue($counter,value){$counter.toggleClass('empty',value==0)
$counter.text(value)}
function setCountersValue(itemId,$el,value){setCounterValue($('span.counter[data-menu-id="'+itemId+'"]',$el),value)
...
SideNav.prototype.setCounter=function(itemId,value){setCountersValue(itemId,this.$el,value)
return this}
...
this.$scrollbar=$('<div />').addClass('scrollbar-scrollbar');this.$track=$('<div />').addClass('scrollbar-track').appendTo(this.$scrollbar);this.$thumb=$('<div />').addClass('scrollbar-thumb').appendTo(this.$track);$el.addClass('drag-scrollbar').addClass(options.vertical?'vertical':'horizontal').prepend(this.$scrollbar);if(isTouch){this.$el.on('touchstart',function(event){var touchEvent=event.originalEvent;if(touchEvent.touches.length==1){startDrag(touchEvent.touches[0]);event.stopPropagation();}})}
else{this.$thumb.on('mousedown',function(event){startDrag(event);});this.$track.on('mouseup',function(event){moveDrag(event);});}
$el.mousewheel(function(event){var offset=self.options.vertical?((event.deltaFactor*event.deltaY)*-1):(event.deltaFactor*event.deltaX);return!scrollWheel(offset*self.options.scrollSpeed);})
$el.on('oc.scrollbar.gotoStart',function(event){self.options.vertical?$el.scrollTop(0):$el.scrollLeft(0);self.update();event.stopPropagation();})
$(window).on('resize',$.proxy(this.update,this));$(window).on('oc.updateUi',$.proxy(this.update,this));function startDrag(event){$('body').addClass('drag-noselect');$el.trigger('oc.scrollStart');dragStart=event[eventElementName];startOffset=self.options.vertical?$el.scrollTop():$el.scrollLeft();if(isTouch){$(window).on('touchmove.scrollbar',function(event){var touchEvent=event.originalEvent
if(moveDrag(touchEvent.touches[0]))
event.preventDefault();});$el.on('touchend.scrollbar',stopDrag);}
else{$(window).on('mousemove.scrollbar',function(event){moveDrag(event);return false;})
$(window).on('mouseup.scrollbar',function(){stopDrag();return false;})}}
function moveDrag(event){self.isLocked=true;var
offset,dragTo=event[eventElementName];if(self.isTouch){offset=dragStart-dragTo;}
else{var ratio=self.getCanvasSize()/self.getViewportSize();offset=(dragTo-dragStart)*ratio;}
self.options.vertical?$el.scrollTop(startOffset+offset):$el.scrollLeft(startOffset+offset);self.setThumbPosition();return self.options.vertical?el.scrollTop!=startOffset:el.scrollLeft!=startOffset;}
function stopDrag(){$('body').removeClass('drag-noselect');$el.trigger('oc.scrollEnd');$(window).off('.scrollbar');}
var isWebkit=$(document.documentElement).hasClass('webkit');function scrollWheel(offset){startOffset=self.options.vertical?el.scrollTop:el.scrollLeft;$el.trigger('oc.scrollStart');self.options.vertical?$el.scrollTop(startOffset+offset):$el.scrollLeft(startOffset+offset);var scrolled=self.options.vertical?el.scrollTop!=startOffset:el.scrollLeft!=startOffset;self.setThumbPosition();if(!isWebkit){if(self.endScrollTimeout!==undefined){clearTimeout(self.endScrollTimeout);self.endScrollTimeout=undefined;}
self.endScrollTimeout=setTimeout(function(){$el.trigger('oc.scrollEnd');self.endScrollTimeout=undefined;},50);}
else{$el.trigger('oc.scrollEnd');}
return scrolled;}
setTimeout(function(){self.update()},1);}
Scrollbar.prototype=Object.create(BaseProto);Scrollbar.prototype.constructor=Scrollbar;Scrollbar.prototype.dispose=function(){this.unregisterHandlers();BaseProto.dispose.call(this);}
...
this.$scrollbar.hide();this.setThumbSize();this.setThumbPosition();this.$scrollbar.show();}
Scrollbar.prototype.setThumbSize=function(){var properties=this.calculateProperties()
this.isScrollable=!(properties.thumbSizeRatio>=1);this.$scrollbar.toggleClass('disabled',!this.isScrollable);if(this.options.vertical){this.$track.height(properties.canvasSize);this.$thumb.height(properties.thumbSize);}
else{this.$track.width(properties.canvasSize);this.$thumb.width(properties.thumbSize);}}
Scrollbar.prototype.setThumbPosition=function(){var properties=this.calculateProperties();if(this.options.vertical){this.$thumb.css({top:properties.thumbPosition});}
else{this.$thumb.css({left:properties.thumbPosition});}}
Scrollbar.prototype.calculateProperties=function(){var $el=this.$el,properties={};properties.viewportSize=this.getViewportSize();properties.canvasSize=this.getCanvasSize();properties.scrollAmount=(this.options.vertical)?$el.scrollTop():$el.scrollLeft();var offset=1;properties.thumbSizeRatio=properties.viewportSize/(properties.canvasSize-offset);properties.thumbSize=properties.viewportSize*properties.thumbSizeRatio;properties.thumbPositionRatio=properties.scrollAmount/(properties.canvasSize-properties.viewportSize);properties.thumbPosition=((properties.viewportSize-properties.thumbSize)*properties.thumbPositionRatio)+properties.scrollAmount;if(isNaN(properties.thumbPosition)){properties.thumbPosition=0}
return properties;}
...
Scrollbar.prototype.gotoElement=function(element,callback){var $el=$(element);if(!$el.length){return;}
var self=this,offset=0,animated=false,params={duration:300,queue:false,complete:function(){if(callback!==undefined){callback();}}}
if(!this.options.vertical){offset=$el.get(0).offsetLeft-this.$el.scrollLeft();if(offset<0){this.$el.animate({'scrollLeft':$el.get(0).offsetLeft},params);animated=true;}
else{offset=$el.get(0).offsetLeft+$el.outerWidth()-(this.$el.scrollLeft()+this.$el.outerWidth());if(offset>0){this.$el.animate({'scrollLeft':$el.get(0).offsetLeft+$el.outerWidth()-this.$el.outerWidth()},params);animated=true;}}}
else{offset=$el.get(0).offsetTop-this.$el.scrollTop();if(this.options.animation){if(offset<0){this.$el.animate({'scrollTop':$el.get(0).offsetTop},params);animated=true;}
else{offset=$el.get(0).offsetTop-(this.$el.scrollTop()+this.$el.outerHeight());if(offset>0){this.$el.animate({'scrollTop':$el.get(0).offsetTop+$el.outerHeight()-this.$el.outerHeight()},params);animated=true;}}}
else{if(offset<0){this.$el.scrollTop($el.get(0).offsetTop);}
else{offset=$el.get(0).offsetTop-(this.$el.scrollTop()+this.$el.outerHeight())
if(offset>0){this.$el.scrollTop($el.get(0).offsetTop+$el.outerHeight()-this.$el.outerHeight());}}}}
if(!animated&&callback!==undefined){callback();}
return this}
Scrollbar.prototype.dispose=function(){this.$el=null;this.$scrollbar=null;this.$track=null;this.$thumb=null;}
var old=$.fn.scrollbar
$.fn.scrollbar=function(option){return this.each(function(){var $this=$(this);var data=$this.data('oc.scrollbar');var options=$.extend({},Scrollbar.DEFAULTS,$this.data(),typeof option=='object'&&option);if(!data)$this.data('oc.scrollbar',(data=new Scrollbar(this,options)));if(typeof option=='string')data[option].call($this);});}
$.fn.scrollbar.Constructor=Scrollbar;$.fn.scrollbar.noConflict=function(){$.fn.scrollbar=old;return this;}
...
return this}
$(document).ready(function(){$('[data-control=filelist]').fileList()})}(window.jQuery);(function($){var OctoberLayout=function(){}
...
if($el.find('.drag-handle').length>0){sortableOptions.handle='.drag-handle'}
...
return this}
$(document).render(function(){$('[data-control="treelist"]').treeListWidget();})}(window.jQuery);+function($){"use strict";var SidenavTree=function(element,options){this.options=options;this.$el=$(element);this.init();}
SidenavTree.DEFAULTS={treeName:'sidenav_tree',rememberSearch:false}
SidenavTree.prototype.init=function(){var self=this;$(document.body).addClass('has-sidenav-tree');this.statusCookieName=this.options.treeName+'groupStatus';this.searchCookieName=this.options.treeName+'search';this.$searchInput=$(this.options.searchInput);this.$el.on('click','li > div.group',function(){self.toggleGroup($(this).closest('li'));return false;})
this.$searchInput.on('input',function(){self.handleSearchChange();})
if(this.options.rememberSearch){var searchTerm=$.cookie(this.searchCookieName);if(searchTerm!==undefined&&searchTerm.length>0){this.$searchInput.val(searchTerm);this.applySearch();}}
var scrollbar=$('[data-control=scrollbar]',this.$el).data('oc.scrollbar'),active=$('li.active',this.$el);if(active.length>0){scrollbar.gotoElement(active);}}
SidenavTree.prototype.toggleGroup=function(group){var $group=$(group),status=$group.attr('data-status');status===undefined||status=='expanded'?this.collapseGroup($group):this.expandGroup($group);}
SidenavTree.prototype.collapseGroup=function(group){var
$list=$('> ul',group),self=this;$list.css('overflow','hidden');$list.animate({'height':0},{duration:100,queue:false,complete:function(){$list.css({'overflow':'visible','display':'none'})
$(group).attr('data-status','collapsed')
$(window).trigger('oc.updateUi')
self.saveGroupStatus($(group).data('group-code'),true)}});}
SidenavTree.prototype.expandGroup=function(group,duration){var
$list=$('> ul',group),self=this;duration=duration===undefined?100:duration;$list.css({'overflow':'hidden','height':0});$list.animate({'height':$list[0].scrollHeight},{duration:duration,queue:false,complete:function(){$list.css({'overflow':'visible','height':'auto','display':''});$(group).attr('data-status','expanded');$(window).trigger('oc.updateUi');self.saveGroupStatus($(group).data('group-code'),false);}})}
SidenavTree.prototype.saveGroupStatus=function(groupCode,collapsed){var collapsedGroups=$.cookie(this.statusCookieName),updatedGroups=[];if(collapsedGroups===undefined){collapsedGroups='';}
collapsedGroups=collapsedGroups.split('|');$.each(collapsedGroups,function(){if(groupCode!=this){updatedGroups.push(this);}})
...
this.$searchInput.data('oc.lastvalue',this.$searchInput.val());if(this.dataTrackInputTimer!==undefined){window.clearTimeout(this.dataTrackInputTimer);}
var self=this;this.dataTrackInputTimer=window.setTimeout(function(){self.applySearch();},300);$.cookie(this.searchCookieName,$.trim(this.$searchInput.val()),{expires:30,path:'/'});}
SidenavTree.prototype.applySearch=function(){var query=$.trim(this.$searchInput.val()),words=query.toLowerCase().split(' '),visibleGroups=[],visibleItems=[],self=this;if(query.length==0){$('li',this.$el).removeClass('hidden');this.$el.removeClass('is-searching');return;}
this.$el.addClass('is-searching');$('ul.top-level > li',this.$el).each(function(){var $li=$(this);if(self.textContainsWords($('div.group h3',$li).text(),words)){visibleGroups.push($li.get(0));$('ul li',$li).each(function(){visibleItems.push(this);});}
else{$('ul li',$li).each(function(){if(self.textContainsWords($(this).text(),words)||self.textContainsWords($(this).data('keywords'),words)){visibleGroups.push($li.get(0));visibleItems.push(this);}})}})
$('ul.top-level > li',this.$el).each(function(){var $li=$(this),groupIsVisible=$.inArray(this,visibleGroups)!==-1;$li.toggleClass('hidden',!groupIsVisible);if(groupIsVisible){self.expandGroup($li,0);}
$('ul li',$li).each(function(){var $itemLi=$(this);$itemLi.toggleClass('hidden',$.inArray(this,visibleItems)==-1);})});$(window).trigger('resize');return false;}
SidenavTree.prototype.textContainsWords=function(text,words){text=text.toLowerCase();for(var i=0;i<words.length;i++){if(text.indexOf(words[i])===-1){return false;}}
return true;}
...
return this}
$(document).render(function(){$('time[data-datetime-control]').dateTimeConverter()})}(window.jQuery);+function($){"use strict";function ResponsiveMenu(closeCallback){var $mainMenuElement=$('#layout-mainmenu .navbar ul.mainmenu-items'),$menuContainer=$mainMenuElement.closest('.layout-row'),$responsiveMenuContainer=$('#layout-mainmenu-responsive-container'),$responsiveMainMenuPane=$responsiveMenuContainer.find('.mainmenu-pane'),$responsiveSubMenuPane=$responsiveMenuContainer.find('.submenu-pane'),$responsiveMainMenu=$responsiveMainMenuPane.find('ul.mainmenu-items'),$responsiveSubMenu=$responsiveSubMenuPane.find('ul.mainmenu-items'),$subMenuHeader=$responsiveSubMenuPane.find('.menu-header'),$subMenuHeaderMenuItem=$subMenuHeader.find('.mainmenu-item')
function init(){if(!$mainMenuElement.length){return}
setupResponsiveMenu()
initListeners()}
function updateScrollIndicator(el){$(el.parentElement).toggleClass('scrolled',el.scrollTop>0);}
function initScrollablePanel(el){el.addEventListener('scroll',function(evt){updateScrollIndicator(evt.target)},{capture:true,passive:true})}
function initListeners(){initScrollablePanel($responsiveMainMenu.get(0))
...
function initMenuScroll($menu){$menu.dragScroll({vertical:true,useNative:false,useDrag:true})}
function setupResponsiveMenu(){initMenuScroll($responsiveMainMenu)
initMenuScroll($responsiveSubMenu)
$(document.body).on('click','.mainmenu-items',function(){if($(document.body).hasClass('drag')){return false}})}
function onMainMenuItemClick(ev){var $menuItem=$(ev.currentTarget),$item=$menuItem.closest('.mainmenu-item')
if(!$item.hasClass('has-subitems')){return}
var submenuIndex=$item.data('submenuIndex'),$submenu=$menuContainer.find('.mainmenu-submenu-dropdown[data-submenu-index='+submenuIndex+']')
...
ev.preventDefault()}
function onCloseSubmenuClick(ev){$(document.body).removeClass('responsive-submenu-displayed')
ev.preventDefault()}
function onMenuMenuCloseClick(){closeCallback()}
this.show=function(){$responsiveMainMenu.dragScroll('goToStart')
$responsiveSubMenu.dragScroll('goToStart')
$(document.body).addClass('responsive-menu-displayed')}
this.hide=function(){$(document.body).removeClass('responsive-menu-displayed');window.setTimeout(function(){$(document.body).removeClass('responsive-submenu-displayed')},250)}
init()}
$.oc.responsiveMenu=ResponsiveMenu}(window.jQuery);+function($){"use strict";function MainMenu(){var $mainMenuElement=$('#layout-mainmenu .navbar ul.mainmenu-items'),$mainMenuToolbar=$('#layout-mainmenu [data-control=toolbar]'),$menuContainer=$mainMenuElement.closest('.layout-row'),menuHeight=$menuContainer.closest('.main-menu-container').outerHeight(),responsiveMenu=new $.oc.responsiveMenu(hideMenus),$overlay=null
function init(){$mainMenuElement.on('click','li.has-subitems',onItemClick)
$mainMenuElement.on('click','.mainmenu-toggle',onShowResponsiveMenuClick)
var dragScroll=$mainMenuToolbar.data('oc.dragScroll')
if(dragScroll){dragScroll.goToElement($('> li.active',$mainMenuElement),undefined,{'duration':0})}}
function displaySubmenu($li){var submenuIndex=$li.data('submenuIndex'),$submenu=$menuContainer.find('.mainmenu-submenu-dropdown[data-submenu-index='+submenuIndex+']'),menuLeft=$li.offset().left
getOverlay().addClass('show')
$submenu.css({top:menuHeight,left:menuLeft})
$submenu.addClass('invisible')
$submenu.addClass('show')
var menuRight=menuLeft+$submenu.width(),windowWidth=$(window).width()
if(menuRight>windowWidth-20){$submenu.css({left:menuLeft-(menuRight-windowWidth)-20})}
$submenu.removeClass('invisible')
addKeyListener()}
function onKeyDown(ev){if(ev.keyCode==27){hideMenus()}}
function addKeyListener(){$(document).on('keydown.mainmenusubmenu',onKeyDown)}
function removeKeyListener(){$(document).off('.mainmenusubmenu')}
function hideSubmenu(){$menuContainer.find('.mainmenu-submenu-dropdown.show').removeClass('show')}
function hideMenus(){getOverlay().removeClass('show')
hideSubmenu()
responsiveMenu.hide()
removeKeyListener()}
function getOverlay(){if($overlay){return $overlay}
$overlay=$('<div class="mainmenu-submenu-overlay"></div>').appendTo(document.body)
$overlay.on('click',hideMenus)
return $overlay}
function onItemClick(ev){var $li=$(ev.currentTarget).closest('li')
ev.preventDefault()
if($(document.body).hasClass('drag')){return false}
displaySubmenu($li)
return false}
function onShowResponsiveMenuClick(ev){ev.preventDefault()
addKeyListener()
getOverlay().addClass('show')
responsiveMenu.show()
return false}
init()}
$(document).ready(function(){$.oc.mainMenu=new MainMenu()})}(window.jQuery);+(function($){'use strict';var ModalFocusManager=function(){var modalStack=[];function addEventAndCall(callback){$(document).on('focusin.octoberfocusmanager',callback);callback(null);}
function removeCallbacks(){$(document).off('.octoberfocusmanager');}
function findIndexByUid(uid){for(var index=0;index<modalStack.length;index++){if(modalStack[index].uid==uid){return index;}}
return null;}
this.push=function(focusInCallback,type,uid,isHotkeyBlocking){removeCallbacks();addEventAndCall(focusInCallback);modalStack.push({focusInCallback:focusInCallback,type:type,uid:uid,isHotkeyBlocking:isHotkeyBlocking});};this.hasHotkeyBlockingAbove=function(uid){var uidIndex=findIndexByUid(uid),startIndex=uidIndex!==null?uidIndex:0;for(var index=startIndex;index<modalStack.length;index++){if(modalStack[index].isHotkeyBlocking){return true;}}
return false;};this.getTop=function(){if(modalStack.length>0){return modalStack[modalStack.length-1];}
return null;};this.isUidTop=function(uid){var top=this.getTop();if(top===null||top.uid===undefined){return false;}
return top.uid===uid;};this.getNumberOfType=function(type){var result=0;for(var index=0;index<modalStack.length;index++){if(modalStack[index].type==type){result++;}}
return result;};this.pop=function(){modalStack.pop();removeCallbacks();if(modalStack.length>0){var callback=modalStack[modalStack.length-1].focusInCallback;addEventAndCall(callback);}};};$.oc.modalFocusManager=new ModalFocusManager();})(window.jQuery);+function($){"use strict";var DomIdManager=function(){var indexes=1;this.generate=function(prefix){var result='id-'+(indexes++);if(typeof prefix=='string'){result=prefix+'-'+result;}
return result;};};$.oc.domIdManager=new DomIdManager();}(window.jQuery);+(function($){'use strict';var VueUtils=function(){function findTraverseObjects(objectArray,currentKey,keyProperty){for(var index=0;index<objectArray.length;index++){if(objectArray[index][keyProperty]===currentKey){return objectArray[index];}}
return null;}
this.getObjectProperty=function(obj,propertyPath){var pathParts=propertyPath.split('.'),currentObject=obj;for(var index=0;index<pathParts.length;index++){var currentProperty=pathParts[index];if(currentObject[currentProperty]===undefined){return undefined;}
currentObject=currentObject[currentProperty];}
return currentObject;};this.getCleanObject=function(obj){return JSON.parse(JSON.stringify(obj));};this.findObjectByKeyPath=function(objectArray,keyPathArray,childrenListProperty,keyProperty){var keyPathCopy=keyPathArray.slice(),currentKey=keyPathCopy.shift(),obj=null;while(currentKey!==undefined){if(!objectArray){return null;}
obj=findTraverseObjects(objectArray,currentKey,keyProperty);if(obj===null){return obj;}
objectArray=this.getObjectProperty(obj,childrenListProperty);if(objectArray===undefined){objectArray=[];}
currentKey=keyPathCopy.shift();}
return obj;};this.findObjectsByProperty=function(objectArray,childrenListProperty,propertyName,propertyValue,path){var result=[];function traverse(objects){for(var index=0;index<objects.length;index++){var object=objects[index];if(object[propertyName]===propertyValue){result.push(object);}
traverse(object[childrenListProperty]||[]);}}
traverse(objectArray);return result;};this.getObjectPathByProperty=function(objectArray,childrenListProperty,propertyName,propertyValue){function traverse(objects,path){for(var index=0;index<objects.length;index++){var object=objects[index],currentPropertyValue=object[propertyName];if(currentPropertyValue===propertyValue){return path.concat(currentPropertyValue);}
var childPath=traverse(object[childrenListProperty]||[],path.concat(currentPropertyValue));if(childPath){return childPath;}}}
return traverse(objectArray,[]);};this.findObjectParentInfoByProperty=function(objectArray,childrenListProperty,propertyName,propertyValue){function traverse(objects){for(var index=0;index<objects.length;index++){var object=objects[index];if(object[propertyName]===propertyValue){return{parentArray:objects,index:index,object:object};}
var subtreeResult=traverse(object[childrenListProperty]||[]);if(subtreeResult){return subtreeResult;}}}
return traverse(objectArray);};this.getFlattenNodes=function(objectArray,childrenListProperty){var result=[];function traverse(objects){for(var index=0;index<objects.length;index++){var object=objects[index];traverse(object[childrenListProperty]||[]);result.push(object);}}
traverse(objectArray);return result;};this.stringHashCode=function(value){var hash=0,i,chr;for(i=0;i<value.length;i++){chr=value.charCodeAt(i);hash=(hash<<5)-hash+chr;hash|=0;}
return hash;};this.syncObjectProperties=function(srcObj,destObj){for(var srcProp in srcObj){if(!srcObj.hasOwnProperty(srcProp)){continue;}
destObj[srcProp]=srcObj[srcProp];}
for(var destProp in destObj){if(!destObj.hasOwnProperty(destProp)){continue;}
if(!srcObj.hasOwnProperty(destProp)){delete destObj[destProp];}}};};$.oc.vueUtils=new VueUtils();})(window.jQuery);+(function($){'use strict';var Tooltips=function(){var $tooltipElement=null;var tooltipTimeout=null;function init(){addListeners();}
function addListeners(){document.addEventListener('mouseenter',onMouseEnter,true);document.addEventListener('mouseleave',onMouseLeave,true);document.addEventListener('mousedown',onMouseDown);document.addEventListener('click',onMouseDown);document.addEventListener('keydown',onKeyDown);}
function clearTooltipTimeout(){if(tooltipTimeout!==null){clearTimeout(tooltipTimeout);}
tooltipTimeout=null;}
function createTooltip(element){if(!$tooltipElement){$tooltipElement=$('<div class="october-tooltip tooltip-hidden tooltip-invisible"><span class="tooltip-text"></span><span class="tooltip-hotkey"></span></div>');$(document.body).append($tooltipElement);}
$tooltipElement.find('.tooltip-text').text(element.getAttribute('data-tooltip-text'));var tooltipHotkey=element.getAttribute('data-tooltip-hotkey'),hotkeySpan=$tooltipElement.find('.tooltip-hotkey').html('');if(tooltipHotkey){tooltipHotkey.split(',').forEach(function(hotkeys){hotkeySpan.append($('<i>').text(hotkeys.trim()));});}
$tooltipElement.removeClass('tooltip-hidden');$tooltipElement.css('left',0);var $element=$(element),elementOffset=$element.offset(),elementWidth=$element.outerWidth(),elementHeight=$element.height(),tooltipWidth=$tooltipElement.outerWidth(),bodyWidth=$(document.body).width(),left=Math.round(elementOffset.left+elementWidth/2-tooltipWidth/2);if(left<0){left=15;}
var rightDiff=left+tooltipWidth-bodyWidth;if(rightDiff>0){left-=rightDiff+15;}
$tooltipElement.css({left:left,top:elementOffset.top+elementHeight+5});$tooltipElement.removeClass('tooltip-invisible');}
function destroyTooltip(){if(!$tooltipElement){return;}
$tooltipElement.addClass('tooltip-invisible');setTimeout(function(){$tooltipElement.addClass('tooltip-hidden');},150);}
function onMouseEnter(ev){if(!ev.target||!ev.target.getAttribute||!ev.target.dataset){return;}
if(!ev.target.getAttribute('data-tooltip-text')){return;}
clearTooltipTimeout();destroyTooltip();tooltipTimeout=setTimeout(function(){createTooltip(ev.target);},300);}
function onMouseLeave(ev){if(!ev.target||!ev.target.getAttribute||!ev.target.dataset){return;}
if(!ev.target.getAttribute('data-tooltip-text')){return;}
clearTooltipTimeout();destroyTooltip();}
function onMouseDown(ev){destroyTooltip();}
function onKeyDown(ev){destroyTooltip();}
init();this.clear=function(){clearTooltipTimeout();destroyTooltip();};};var tooltips=new Tooltips();$.oc.octoberTooltips={clear:function(){tooltips.clear();}};})(window.jQuery);+(function($){'use strict';function OctoberModuleRegistry(){this.modules=new Map();this.register=function(namespace,registrationFn){if(this.modules.has(namespace)){throw new Error('Module namespace is already registered: '+namespace);}
this.modules.set(namespace,registrationFn());};this.import=function(namespace){if(!this.exists(namespace)){throw new Error('Module namespace is not registered: '+namespace);}
return this.modules.get(namespace);};this.exists=function(namespace){return this.modules.has(namespace);};}
$.oc.module=new OctoberModuleRegistry();})($);$.ajaxPrefilter(function(options){var token=$('meta[name="csrf-token"]').attr('content')
if(token){if(!options.headers)options.headers={}
options.headers['X-CSRF-TOKEN']=token}})
function unregisterServiceWorkers(){if(location.protocol==='https:'){navigator.serviceWorker.getRegistrations().then(function(registrations){for(var index=0;index<registrations.length;index++){registrations[index].unregister({immediate:true})}});}}
if($.oc===undefined)
$.oc={}
...
window.addEventListener('touchstart',function onFirstTouch(){$.cookie('oc-user-touch',1,{expires:365,path:'/'});window.removeEventListener('touchstart',onFirstTouch,false);$(document.documentElement).addClass('user-touch');},false);if(!!window.MSInputMethodContext&&!!document.documentMode){$(window).on('resize',function(){fixMediaManager()
fixSidebar()})
...
$(document).ready(function(){Promise.config({cancellation:true})
